...
ClassImp(o2::hmpid::HMPIDDCSinfo);

void HMPIDDCSinfo::print() const
{
...
{
  // fill the array of the DPIDs that will be used by HMPID
  // pids should be provided by CCDB

  for (const auto& it : pids) {
    mPids[it] = false;
    mHMPIDDCS[it].makeEmpty();
  }
...
int HMPIDDCSProcessor::process(const gsl::span<const DPCOM> dps)
{
  ...
  for (const auto& it : dps) {
    ...
    if (el == mPids.end()) {
      LOG(INFO) << "DP " << it.id << " not found in HMPIDDCSProcessor, we will not process it";
      continue;
    }
    processDP(it);
    mPids[it.id] = true;
...
int HMPIDDCSProcessor::processDP(const DPCOM& dpcom)
{
  ...
  if (processFlags(flags, dpid.get_alias()) == 0) {
    ...
    if (type == RAW_INT) {
      ...
      if (std::strstr(dpid.get_alias(), "FEACSTATUS") != nullptr) { // DP is FEACSTATUS
        ...
        const auto offs = std::strlen("HMPID_FEACSTATUS_");
      ...
      if (std::strstr(dpid.get_alias(), "HVSTATUS") != nullptr) { // DP is HVSTATUS
        ...
        const auto offs = std::strlen("HMPID_HVSTATUS_SM");
        std::size_t const nn = aliasStr.find_first_of("0123456789", offs);
        std::size_t const mm = aliasStr.find_first_not_of("0123456789", nn);
...
void HMPIDDCSProcessor::finalize()
{
  ...
  for (const auto& it : mPids) {
    ...
    if (type == o2::dcs::RAW_DOUBLE) {
      auto& hmpiddcs = mHMPIDDCS[it.first];
      if (it.second == true) { // we processed the DP at least 1x
        auto& dpvect = mDpsdoublesmap[it.first];
        hmpiddcs.firstValue.first = dpvect[0].get_epoch_time();
        converter0.raw_data = dpvect[0].payload_pt1;
        hmpiddcs.firstValue.second = converter0.double_value;
        hmpiddcs.lastValue.first = dpvect.back().get_epoch_time();
        converter0.raw_data = dpvect.back().payload_pt1;
        hmpiddcs.lastValue.second = converter0.double_value;
        ...
        if (dpvect.size() > 1) {
          ...
          if (deltatime < 60000) {
            ...
            hmpiddcs.maxChange.first = deltatime; // is it ok to do like this, as in Run 2?
            hmpiddcs.maxChange.second = delta;
          } else {
            for (auto i = 0; i < dpvect.size() - 1; ++i) {
              for (auto j = i + 1; j < dpvect.size(); ++j) {
                ...
                if (deltatime >= 60000) { // we check every min; epoch_time in ms
                  ...
                  double delta = std::abs(converter0.double_value - converter1.double_value);
                  if (delta > hmpiddcs.maxChange.second) {
                    hmpiddcs.maxChange.first = deltatime; // is it ok to do like this, as in Run 2?
                    hmpiddcs.maxChange.second = delta;
                  }
                }
          ...
          hmpiddcs.midValue.first = dpvect[midIdx].get_epoch_time();
          converter0.raw_data = dpvect[midIdx].payload_pt1;
          hmpiddcs.midValue.second = converter0.double_value;
        } else {
          hmpiddcs.maxChange.first = dpvect[0].get_epoch_time();
          converter0.raw_data = dpvect[0].payload_pt1;
          hmpiddcs.maxChange.second = converter0.double_value;
          hmpiddcs.midValue.first = dpvect[0].get_epoch_time();
          converter0.raw_data = dpvect[0].payload_pt1;
          hmpiddcs.midValue.second = converter0.double_value;
        }
      }
  ...
  prepareCCDBobjectInfo(mHMPIDDCS, mccdbDPsInfo, "HMPID/DCSDPs", mStartTF, md);
...
void HMPIDDCSProcessor::updateFEACCCDB()
{
  ...
  std::map<std::string, std::string> md;
  prepareCCDBobjectInfo(mFeac, mccdbLVInfo, "HMPID/LVStatus", mTF, md);
  return;
}

//______________________________________________________________________

void HMPIDDCSProcessor::updateHVCCDB()
{
  ...
  std::map<std::string, std::string> md;
  prepareCCDBobjectInfo(mHV, mccdbHVInfo, "HMPID/HVStatus", mTF, md);
  return;
}

//_______________________________________________________________________

void HMPIDDCSProcessor::getStripsConnectedToFEAC(int nDDL, int nFEAC, HMPIDFEACinfo& info) const
{

  //
  // Taken from AliRoot/HMPID/AliHMPIDLvHvDataPoints.cxx
  //
  // FEAC-strip mapping:
