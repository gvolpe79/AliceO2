...
#include "HMPIDCalibration/HMPIDChannelCalibrator.h"
...
void HMPIDChannelData::fill(const gsl::span<const o2::dataformats::CalibInfoHMPID> data)
{
  // fill container
  for (int i = data.size(); i--;) {
    auto ch = data[i].getHMPIDChIndex();
    int sector = ch / o2::hmpid::Geo::NPADSXSECTOR;
    int chInSect = ch % o2::hmpid::Geo::NPADSXSECTOR;
    ...
    auto corr = mCalibHMPIDapi->getTimeCalibration(ch, tot); // we take into account LHCphase, offsets and time slewing
...
//_____________________________________________
void HMPIDChannelData::merge(const HMPIDChannelData* prev)
{
  // merge data of 2 slots
  for (int isect = 0; isect < o2::hmpid::Geo::NSECTORS; isect++) {
    mHisto[isect] += prev->getHisto(isect);
  }
  for (int ich = 0; ich < o2::hmpid::Geo::NCHANNELS; ich++) {
    mEntries[ich] += prev->mEntries[ich];
  }
}

//_____________________________________________
bool HMPIDChannelData::hasEnoughData(int minEntries) const
{
...
//_____________________________________________
void HMPIDChannelData::print() const
{
...
//_____________________________________________
void HMPIDChannelData::print(int isect) const
{
...
//_____________________________________________
void HMPIDChannelData::printEntries() const
{
...
//_____________________________________________
int HMPIDChannelData::findBin(float v) const
{
...
//_____________________________________________
float HMPIDChannelData::integral(int chmin, int chmax, float binmin, float binmax) const
{
  // calculates the integral in [chmin, chmax] and in [binmin, binmax]

  if (binmin < -mRange || binmax > mRange || chmin < 0 || chmax >= o2::hmpid::Geo::NCHANNELS) {
  ...
  int sector = chmin / o2::hmpid::Geo::NPADSXSECTOR;
  if (sector != chmax / o2::hmpid::Geo::NPADSXSECTOR) {
    throw std::runtime_error("We cannot integrate over channels that belong to different sectors");
  }

  int chinsectormin = chmin % o2::hmpid::Geo::NPADSXSECTOR;
  int chinsectormax = chmax % o2::hmpid::Geo::NPADSXSECTOR;
...
//_____________________________________________
float HMPIDChannelData::integral(int ch, float binmin, float binmax) const
{
...
//_____________________________________________
float HMPIDChannelData::integral(int chmin, int chmax, int binxmin, int binxmax) const
{
  // calculates the integral in [chmin, chmax] and in [binmin, binmax]

  if (binxmin < 0 || binxmax > mNBins || chmin < 0 || chmax >= o2::hmpid::Geo::NCHANNELS) {
    throw std::runtime_error("Check your bins, we cannot calculate the integrals in under/overflows bins");
  }
...
//_____________________________________________
float HMPIDChannelData::integral(int ch, int binxmin, int binxmax) const
{
  // calculates the integral along one fixed channel and in [binmin, binmax]
...
//_____________________________________________
float HMPIDChannelData::integral(int ch) const
{
...
//_____________________________________________
void HMPIDChannelCalibrator::initOutput()
{
...
//_____________________________________________
bool HMPIDChannelCalibrator::hasEnoughData(const Slot& slot) const
{

  // Checking if all channels have enough data to do calibration.
  // Delegating this to HMPIDChannelData

  const o2::hmpid::HMPIDChannelData* c = slot.getContainer();
  LOG(INFO) << "Checking statistics";
  return (mTest ? true : c->hasEnoughData(mMinEntries));
}

//_____________________________________________
void HMPIDChannelCalibrator::finalizeSlot(Slot& slot)
{
  // Extract results for the single slot
  o2::hmpid::HMPIDChannelData* c = slot.getContainer();
  LOG(INFO) << "Finalize slot " << slot.getTFStart() << " <= TF <= " << slot.getTFEnd();

  // for the CCDB entry
  std::map<std::string, std::string> md;
  TimeSlewing& ts = mCalibHMPIDapi->getSlewParamObj(); // we take the current CCDB object, since we want to simply update the offset

  for (int ich = 0; ich < o2::hmpid::Geo::NCHANNELS; ich++) {
    // make the slice of the 2D histogram so that we have the 1D of the current channel
    int sector = ich / o2::hmpid::Geo::NPADSXSECTOR;
    int chinsector = ich % o2::hmpid::Geo::NPADSXSECTOR;
    ...
    */
    ...
    ts.setFractionUnderPeak(ich / o2::hmpid::Geo::NPADSXSECTOR, ich % o2::hmpid::Geo::NPADSXSECTOR, fractionUnderPeak);
    ts.setSigmaPeak(ich / o2::hmpid::Geo::NPADSXSECTOR, ich % o2::hmpid::Geo::NPADSXSECTOR, abs(fitValues[2]));
    ts.updateOffsetInfo(ich, fitValues[1]);
  }
  auto clName = o2::utils::MemFileHelper::getClassName(ts);
  auto flName = o2::ccdb::CcdbApi::generateFileName(clName);
  mInfoVector.emplace_back("HMPID/ChannelCalib", clName, flName, md, slot.getTFStart(), 99999999999999);
  mTimeSlewingVector.emplace_back(ts);
}

//_____________________________________________
Slot& HMPIDChannelCalibrator::emplaceNewSlot(bool front, TFType tstart, TFType tend)
{

  auto& cont = getSlots();
  auto& slot = front ? cont.emplace_front(tstart, tend) : cont.emplace_back(tstart, tend);
  slot.setContainer(std::make_unique<HMPIDChannelData>(mNBins, mRange, mCalibHMPIDapi));
  return slot;
}
