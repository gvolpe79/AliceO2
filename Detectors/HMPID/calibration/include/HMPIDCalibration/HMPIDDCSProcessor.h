...
#ifndef DETECTOR_HMPIDDCSPROCESSOR_H_
#define DETECTOR_HMPIDDCSPROCESSOR_H_
...
struct HMPIDDCSinfo {
  std::pair<uint64_t, double> firstValue; // first value seen by the HMPID DCS processor
  std::pair<uint64_t, double> lastValue;  // last value seen by the HMPID DCS processor
  std::pair<uint64_t, double> midValue;   // mid value seen by the HMPID DCS processor
  std::pair<uint64_t, double> maxChange;  // maximum variation seen by the HMPID DCS processor
  HMPIDDCSinfo()
  {
  ...
  ClassDefNV(HMPIDDCSinfo, 1);
};

struct HMPIDFEACinfo {
...
class HMPIDDCSProcessor
{
 ...
 public:
  ...
  void finalize();
  void getStripsConnectedToFEAC(int nDDL, int nFEAC, HMPIDFEACinfo& info) const;
  void updateFEACCCDB();
  void updateHVCCDB();

  const CcdbObjectInfo& getccdbDPsInfo() const { return mccdbDPsInfo; }
  CcdbObjectInfo& getccdbDPsInfo() { return mccdbDPsInfo; }
  const std::unordered_map<DPID, HMPIDDCSinfo>& getHMPIDDPsInfo() const { return mHMPIDDCS; }
 ...
 private:
  std::unordered_map<DPID, HMPIDDCSinfo> mHMPIDDCS;                // this is the object that will go to the CCDB
  ...
  std::array<std::array<HMPIDFEACinfo, NFEACS>, Geo::kNDDL> mFeacInfo;                  // contains the strip/pad info per FEAC
...
template <typename T>
void HMPIDDCSProcessor::prepareCCDBobjectInfo(T& obj, CcdbObjectInfo& info, const std::string& path, TFType tf,
                                            const std::map<std::string, std::string>& md)
{
