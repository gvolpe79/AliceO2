...
public:
//ctor&dtor    
  virtual        ~Param() {if (fgInstance){for(Int_t i=0;i<7;i++){delete fM[i];fM[i] = 0x0;};fgInstance=0;}}
...
//geo info
  ...
  static Int_t  InHVSector(           Float_t y     );                                                           //find HV sector
...
//
  ...
  static Float_t fgCellX, fgCellY, fgPcX, fgPcY, fgAllX, fgAllY;                                   //definition of HMPID geometric parameters 
         Param(Bool_t noGeo);             //default ctor is protected to enforce it to be singleton
